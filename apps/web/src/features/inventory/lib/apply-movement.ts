...
import type { MovementType, MovementReason, Prisma } from "@nohub/db";
...
export type MovementResult =
  ...
  | { success: false; error: "DUPLICATE" | "INSUFFICIENT_STOCK" | "NEGATIVE_BALANCE" | "UNKNOWN"; message: string };
...
export async function applyMovement(input: MovementInput): Promise<MovementResult> {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      // ── 4. Atualiza saldo (upsert) ───────────────────────────────
      await tx.stockBalance.upsert({
        where: {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          organizationId_productId_variantId_locationId_lotId: balanceKey as any,
        },
        create: {
          ...balanceKey,
    ...
    if (e.code === "INSUFFICIENT_STOCK") {
      return { success: false, error: "INSUFFICIENT_STOCK", message: "Estoque disponível insuficiente" };
    }
    if (e.code === "NEGATIVE_BALANCE") {
      return { success: false, error: "NEGATIVE_BALANCE", message: "Saldo não pode ficar negativo" };
    }
...
async function getBalance(input: Pick<MovementInput, "organizationId" | "productId" | "variantId" | "locationId" | "lotId">) {
