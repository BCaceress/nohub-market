...
import { decryptCredentials, decryptCertificate } from "./crypto-helpers";
...
export async function processIssuance(
  ...
  if (certificate.validTo < new Date()) {
    return { success: false, error: "Certificado A1 vencido — faça upload de um novo certificado", retryable: false };
  }

  // ── 3. Decifrar credenciais e certificado ─────────────────────
  const providerCreds = fiscalConfig.providerCredentials
    ? decryptCredentials(fiscalConfig.providerCredentials)
    : null;
  ...
  const pfxResult = decryptCertificate(certificate.encryptedPfx, certificate.encryptedPassword, certificate.iv);
  ...
  const draftResult = buildInvoiceFromOrder({
    ...
    fiscalConfig: {
      ...
      nfceCscToken: fiscalConfig.nfceCscToken,
  ...
  const providerConfig = {
    environment: fiscalConfig.environment === "PRODUCTION" ? "production" as const : "homologation" as const,
    cnpj:        invoice.organization.document.replace(/\D/g, ""),
    token:       providerCreds.token as string,
    csc:         fiscalConfig.nfceCscToken
                   ? fiscalConfig.nfceCscToken.toString("utf8")
                   : "",
